...
import { SiInstagram, SiKakao } from 'react-icons/si';
...
const formatTimeRange = (label: string, startHour: string, startMinute: string, endHour: string, endMinute: string) =>
    ...
    <Layout>
      <section id="hero" className="relative flex min-h-[90vh] items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src={IMAGES.GALLERY_1} alt="네일아트 배경" className="h-full w-full object-cover opacity-30" />
        ...
        <div className="container relative z-10 mx-auto px-4 text-center">
          ...
          >
            ...
            <p className="mx-auto mb-12 max-w-2xl text-xl text-muted-foreground md:text-2xl">
              섬세한 감각과 트렌디한 디자인으로 완성하는 프리미엄 네일 케어
            </p>
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              <Button size="lg" className="rounded-2xl px-8 py-6 text-lg shadow-lg transition-all duration-200 hover:scale-105 hover:shadow-xl" asChild>
      ...
      <section id="services" className="bg-gradient-to-b from-background to-muted/20 py-24">
        <div className="container mx-auto px-4">
          <motion.div className="mb-16 text-center" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 300, damping: 35 }}>
            <h2 className="mb-4 text-4xl font-bold text-foreground md:text-5xl">프리미엄 서비스</h2>
            <p className="mx-auto max-w-2xl text-lg text-muted-foreground">전문 케어와 감각적인 디자인으로 완성하는 맞춤 네일 케어</p>
          </motion.div>

          <motion.div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4" variants={staggerContainer} initial="initial" whileInView="animate" viewport={{ once: true }}>
      ...
      <section id="pricing" className="bg-gradient-to-b from-muted/20 to-background py-24">
        <div className="container mx-auto px-4">
          <motion.div className="mb-16 text-center" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 300, damping: 35 }}>
            ...
            <p className="mx-auto max-w-2xl text-lg text-muted-foreground">합리적인 가격으로 만나는 프리미엄 네일 서비스</p>
          </motion.div>

          <motion.div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3" variants={staggerContainer} initial="initial" whileInView="animate" viewport={{ once: true }}>
            {priceItems.map((item) => (
      ...
      <section id="testimonials" className="bg-gradient-to-b from-background to-muted/20 py-24">
        <div className="container mx-auto px-4">
          <motion.div className="mb-16 text-center" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 300, damping: 35 }}>
            ...
            <p className="mx-auto max-w-2xl text-lg text-muted-foreground">실제 고객이 남긴 후기 중 최근 9개를 먼저 보여드립니다.</p>
          </motion.div>
      ...
      <section id="contact" className="bg-gradient-to-b from-muted/20 to-background py-24">
        <div className="container mx-auto px-4">
          <motion.div className="mb-16 text-center" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 300, damping: 35 }}>
            ...
            <p className="mx-auto max-w-2xl text-lg text-muted-foreground">편안한 공간에서 디테일한 케어를 경험해 보세요.</p>
          </motion.div>

          <div className="mx-auto grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-2">
            <motion.div className="h-full" initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 300, damping: 35 }}>
                ...
                <div className="space-y-6">
                  ...
                  <div className="border-t border-border pt-6">
                    ...
                    <div className="flex gap-4">
                      <a href={siteSettings.instagramUrl} target="_blank" rel="noopener noreferrer" className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 transition-all duration-200 hover:scale-110 hover:bg-primary/20">
                        <SiInstagram className="h-6 w-6 text-primary" />
                      </a>
                      <a href={siteSettings.kakaoOpenChatUrl} target="_blank" rel="noopener noreferrer" className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 transition-all duration-200 hover:scale-110 hover:bg-primary/20">
                        <SiKakao className="h-6 w-6 text-primary" />
                      </a>
                    </div>
                  </div>
                </div>
      ...
      <TestimonialDetailDialog testimonial={selectedTestimonial} open={!!selectedTestimonial} onOpenChange={(open) => !open && setSelectedTestimonial(null)} showActions={false} />
    </Layout>
  );
}
